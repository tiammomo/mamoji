...
/** Budget Controller Unit Tests Tests endpoints without Spring context */
...
public class BudgetControllerTest {
    ...
    void setUp() {
        ...
        mockMvc =
                MockMvcBuilders.standaloneSetup(budgetController)
                        .setControllerAdvice(
    ...
    public void testListBudgets() throws Exception {
        BudgetVO budget =
                createBudgetVO(
                        1L, "Test Budget", new BigDecimal("1000.00"), new BigDecimal("200.00"));
    ...
    public void testListActiveBudgets() throws Exception {
        BudgetVO budget =
                createBudgetVO(
                        1L, "Active Budget", new BigDecimal("1000.00"), new BigDecimal("100.00"));
        when(budgetService.listActiveBudgets(testUserId)).thenReturn(List.of(budget));

        mockMvc.perform(get("/api/v1/budgets").param("activeOnly", "true"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200))
