...
class ReportServiceTest {
    ...
    @Mock private FinTransactionMapper transactionMapper;

    @Mock private FinAccountMapper accountMapper;

    @Mock private FinCategoryMapper categoryMapper;
    ...
    void getSummary_WithValidUser_ReturnsCorrectTotals() {
        ...
        when(transactionMapper.sumAmountByUserTypeAndDateRange(eq(1L), eq("income"), any(), any()))
                .thenReturn(new BigDecimal("10000.00"));

        when(transactionMapper.sumAmountByUserTypeAndDateRange(eq(1L), eq("expense"), any(), any()))
    ...
    void getIncomeExpenseReport_WithValidUser_ReturnsGroupedByCategory() {
        ...
        assertThat(result)
                .anyMatch(
                        vo ->
                                vo.getCategoryName().equals("Salary")
                                        && vo.getAmount().compareTo(new BigDecimal("5000.00"))
                                                == 0);
        assertThat(result)
                .anyMatch(
                        vo ->
                                vo.getCategoryName().equals("Food")
                                        && vo.getAmount().compareTo(new BigDecimal("2000.00"))
                                                == 0);
    }
