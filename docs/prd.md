...
| 功能 | 优先级 | 用户需求 | 详细规则 |
|------|--------|----------|----------|
| 查看退款明细 | P1 | 作为用户，我希望查看一笔支出的所有退款记录 | 1. 点击"退款"按钮弹出详情弹窗<br>2. 显示该交易的退款列表<br>3. 显示已退金额和可退金额 |
| 发起退款 | P0 | 作为用户，我希望对一笔支出进行退款 | 1. 每次退款生成一条退款记录<br>2. 累计退款金额不能超过原交易金额<br>3. 退款后账户余额增加<br>4. 备注自动添加 "退款：" 前缀 |
| 取消退款 | P1 | 作为用户，我希望取消已发起的退款 | 1. 删除退款记录（软删除 status = 0）<br>2. 账户余额相应减少<br>3. 退款记录不可恢复 |

**退款规则：**

```
1. 退款金额 ≤ 原交易金额 - 已退款金额
2. 每次退款生成独立的 fin_refund 记录
3. 退款后自动更新原交易的 refundSummary
4. 账户余额 = 原交易扣款 + 累计退款金额
5. 退款记录软删除 (status = 0)
```
