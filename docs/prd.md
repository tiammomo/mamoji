...
用户偏好设置存储在**前端 localStorage**，后端不存储。
...
| 功能 | 优先级 | 用户需求 | 详细规则 |
|------|--------|----------|----------|
| 账户列表 | P0 | 作为用户，我希望查看所有账户的概览 | 1. 显示账户名称、类型、余额、资产类别<br>2. 支持按状态筛选（正常/禁用）<br>3. 余额按 asset_category 分组汇总 |
| 添加账户 | P0 | 作为用户，我希望添加新账户，以便开始记账 | 1. 支持类型：cash/bank/credit/alipay/wechat<br>2. 资产类别：fund/credit/topup/debt<br>3. 初始余额默认为 0<br>4. 默认计入总资产 |
...
| type | asset_category | 说明 | 余额规则 |
|------|----------------|------|----------|
| cash | fund | 现金 | 正数表示可用现金 |
| bank | fund | 储蓄卡 | 正数表示存款余额 |
| bank | credit | 信用卡 | 负数表示已透支额度 |
| credit | credit | 信用卡 | 负数表示已透支额度 |
| alipay | fund/topup | 支付宝 | 正数表示余额/零钱 |
| wechat | fund/topup | 微信 | 正数表示零钱 |

### 3. 交易记录

| 功能 | 优先级 | 用户需求 | 详细规则 |
|------|--------|----------|----------|
| 记收入 | P0 | 作为用户，我希望快速记录一笔收入 | 1. 选择账户（余额增加）<br>2. 选择分类（收入类）<br>3. 输入金额<br>4. 选择发生时间（默认当前时间）<br>5. 可选填写备注 |
| 记支出 | P0 | 作为用户，我希望快速记录一笔支出 | 1. 选择账户（余额减少）<br>2. 选择分类（支出类）<br>3. 输入金额<br>4. 选择发生时间（默认当前时间）<br>5. 可选填写备注 |
...
| 功能 | 优先级 | 用户需求 | 详细规则 |
|------|--------|----------|----------|
| 创建预算 | P0 | 作为用户，我希望创建月度/年度预算 | 1. 输入预算名称<br>2. 输入预算金额<br>3. 选择周期类型（monthly/yearly）<br>4. 选择开始日期和结束日期<br>5. 预算金额用于控制支出 |
| 预算列表 | P0 | 作为用户，我希望查看所有预算 | 1. 显示预算名称、金额、已花费、进度百分比<br>2. 显示周期和状态<br>3. 按状态筛选（进行中/已完成/已取消） |
